...
//      sbp_register_callback(&state, SBP_MSG_POS_ECEF, &pos_ecefCallback, (void*) this, &pos_ecef_callback_node);
...
//      sbp_register_callback(&state, SBP_MSG_BASELINE_ECEF, &baseline_ecefCallback, (void*) this, &baseline_ecef_callback_node);
        sbp_register_callback(&state, SBP_MSG_BASELINE_NED, &baseline_nedCallback, (void*) this, &baseline_ned_callback_node);
//      sbp_register_callback(&state, SBP_MSG_VEL_ECEF, &vel_ecefCallback, (void*) this, &vel_ecef_callback_node);
        sbp_register_callback(&state, SBP_MSG_VEL_NED, &vel_nedCallback, (void*) this, &vel_ned_callback_node);
...
*/
    ...
    void vel_nedCallback(u16 sender_id, u8 len, u8 msg[], void *context)
    {
        ...
        double vn = vel.n / 1000.0;
        driver->rtk_vel_north = vn;
        double ve = vel.e / 1000.0;
        driver->rtk_vel_east = ve;
        driver->rtk_vel = sqrt(vn * vn + ve * ve);
        driver->rtk_vel_yaw = asin(vn / driver->rtk_vel);
        driver->rtk_hvel_accuracy = vel.h_accuracy;
    ...
    void PIKSI::DiagCB( diagnostic_updater::DiagnosticStatusWrapper &stat )
    {
        ...
        stat.add( "GPS RTK velocity (m/s)", rtk_vel);
        stat.add( "GPS RTK yaw (deg)", rtk_vel_yaw * (180.0/M_PI));
        stat.add( "GPS RTK velocity accuracy", rtk_hvel_accuracy );
